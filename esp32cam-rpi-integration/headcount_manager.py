...
sys.path.append(str(Path(__file__).parent.parent / "rpi-integration"))
from esp32_capture import capture_image_stream
from supabase_client import push_headcount_log, upload_headcount_image
...
class HeadcountManager:
    ...
    def set_episode(self, episode_id: Optional[int]) -> None:
    ...
    def _run(self, trigger_source: str = "auto") -> None:
        try:
            ...
            # Annotated image gets its own file alongside the raw backup
            raw_stem       = capture.saved_path.stem  # esp32_YYYYMMDD_HHMMSS_raw
            annotated_path = self.output_dir / f"{raw_stem.replace('_raw', '')}_annotated.jpg"
            ...
            with ThreadPoolExecutor(max_workers=1) as pool:
                ...
                try:
                    future.result(timeout=SUPABASE_TIMEOUT_S)
                except FuturesTimeoutError:
                    print(f"[Headcount] Supabase log push timed out after {SUPABASE_TIMEOUT_S}s — skipping.")
