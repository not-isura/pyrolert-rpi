...
def upload_headcount_image(image_path: Path, filename: str) -> Optional[str]:
    ...
    try:
        with open(image_path, "rb") as f:
            client.storage.from_("headcount-captures").upload(
                path=filename,
                file=f,
                file_options={"content-type": "image/jpeg", "upsert": "true"},
            )
        url = client.storage.from_("headcount-captures").get_public_url(filename)
        return url
    except Exception as e:
...
def push_headcount_log(
    ...
    try:
        ...
        print(f"[Supabase] Headcount log push failed: {e}")
        return False